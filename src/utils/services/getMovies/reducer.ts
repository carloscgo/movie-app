...
const setDataFromStorage = ({ draft, state, action, name }: any) => {
  draft[name] = uniqBy([
    ...state[name],
    ...getStorage(name, []),
    action.data
  ...
  produce(state, (draft: any) => {
    switch (action.type) {
      ...
      case MOVIES_ACTION_UNFAVORITE:
        draft.favorites = draft.favorites.filter((item: any) => item.id !== action.data.id)
      ...
      case MOVIES_ACTION_UNDELETE:
        draft.deletes = draft.deletes.filter((item: any) => item.id !== action.data.id)
